...
<div class="row cards-body c-card">
    <div class="form-area">
        <div class="form-left">
            <form action="{{$uri}}" id="add_form" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="category" value="{{ encrypt($uri) }}">
                <input type="hidden" name="uri" value="{{ $uri }}">
                @foreach($tool->ContentToolItem as $Item)
                    @if ($Item->input_type == 'input')
                        <div class="form-group">
                            <span class="btm-char-count"><span class="brand-remaining-count">0</span><span>/80</span></span>
                            <label for="ctext">{{$Item->input_title}}:</label>
                            ...
                            <input type="text"
                                   ...
                                   value="{{ (isset($contents[0]['ads'][$Item->slug])? $contents[0]['ads'][$Item->slug] : '') }}">
                                <span class="error" id="{{$Item->slug}}_error"></span>
                    ...
                    @if ($Item->input_type == 'textarea')
                        <div class="form-group">
                            ...
                            <label for="company">{{$Item->input_title}}:</label>
                            <textarea type="text"
                                      name="{{$Item->slug}}"
                                      maxlength="400"
                                      placeholder="{{$Item->input_info_placeholder}}"
                                      id="company"
                                      rows="3"
                                      class="form-control description-remaining">{{ (isset($contents[0]['ads'][$Item->slug])? $contents[0]['ads'][$Item->slug] : '') }}</textarea>
                            <span class="error" id="{{$Item->slug}}_error"></span>
                        </div>
                    @endif
