        ...
        <div class="row">
            <div class="col-lg-3">
                ...
                <form action="google-ad" id="add_form" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <div  class='tag-input' name="tag">
                            <div   v-for='(tag, index) in docData.tag' :key='tag' class='tag-input__tag'>
                                <div v-model="docData.tags">
                        ...
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div   v-for="size in sizes" :key="size.id" >
                                <button v-on:click="makeRed(size.id)" :class="{'red': isActive == size.id}" type="button" class="btn btn-secondary mr-2 rounded bg-white border-0 text-dark outputs-length-button border-box"
                                        v-model="docData.length"
                                        @click="item.length = size.info">
                                  {{size.name}}
                                </button>
                            </div>
                        </div>
                    </div>
                    <button type="button" id="submit_btn" class="btn btn-secondary cancel-button mdl-btn mb-3" data-dismiss="modal" @click="update">Update</button>
                </form>
            </div>
            <div class="col-lg-9">
                <div>
                ...
                <div>
                    <editor-content v-model="docData.doccontent" :editor="editor" class="editor-style"/>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.content = this.docData.doccontent;
            this.item = this.docData;
            ...
            this.editor = new Editor({
                ...
                onUpdate: () => {
                    console.log( this.editor.getHTML())
                    this.item.doccontent =  this.editor.getHTML();
                },
            })
        },
        beforeUnmount() {
        ...
        methods:{
            ...
            update(){
                try {
                    axios.post('/update/' + this.item.id,this.item)
                        .then(res =>{
                            this.showAlert();
            ...
            addTag (event) {
                ...
                if (val.length > 0) {
                    this.docData.tag.push(val)
                    event.target.value = ''
                }
            },
            removeTag (index) {
                this.docData.tag.splice(index, 1)
            },
        }
    }
</script>
...
<style lang="scss" scoped>
    .tiptap-editor button[data-v-4f20be4b]{
